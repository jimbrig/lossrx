...
view_claim_history <- function(claim_id, claims_data = claims_transactional) {
  ...
  close_date <- dplyr::filter(claim_dat, status == "Closed") |>
  ...
  claim_trans_hist <- dplyr::mutate(
    ...
    status_details = case_when(
      incr_status == "NEW -> Open" ~ "Status: Claim Opened",
      incr_status == "Open -> Open" ~ "Status: No Change",
      incr_status == "Closed -> Closed" ~ "Status: No Change",
      incr_status == "Closed -> Open" ~ "Status: Claim Re-Opened",
      incr_status == "Open -> Closed" ~ "Status: Claim Closed"
    ),
    rept_details = case_when(
      incr_rept == 0 ~ "Reported: No Change",
      incr_rept > 0 ~ paste0("Reported: Increased by ", as.character(formattable::currency(.data$incr_rept))),
      incr_rept < 0 ~ paste0("Reported: Decreased by ", as.character(formattable::currency(.data$incr_rept)))
    ),
    paid_details = case_when(
      payment == 0 ~ "Paid: No Change",
      payment > 0 ~ paste0("Paid: Increased by ", as.character(formattable::currency(.data$payment))),
      payment < 0 ~ paste0("Paid: Decreased by ", as.character(formattable::currency(.data$payment)))
    ),
    case_details = case_when(
      incr_case == 0 ~ "Case Reserve: No Change",
      incr_case > 0 ~ paste0("Case Reserve: Increased by ", as.character(formattable::currency(.data$incr_case))),
      incr_case < 0 ~ paste0("Case Reserve: Decreased by ", as.character(formattable::currency(.data$incr_case)))
    )
  ) |>
    dplyr::rowwise() |>
    dplyr::mutate(
